...
def fine_search():
    ...
    eta = np.array([rnd_eta_lambda(-2, -1) for i in range(iters)])
    lambdas = np.array([rnd_eta_lambda(-5, -4) for i in range(iters)])
    
    res = np.empty((0,3), int)
    
    for i in range(iters):
        clf = TwoLayerNetwork(n_epochs=10, l=lambdas[i], eta=eta[i], 
                              n_batch=100, decay_rate=1, rho=0.9, include_train_cost=False)
    ...
    with open('outfile-fine.dat', 'wb') as fp:
        pickle.dump(res, fp)
